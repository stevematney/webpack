				...
				chunks => {
					...
					const addModuleToChunksInfoMap = (
						...
						if (!alreadyValidatedNames.has(name)) {
							...
							if (compilation.entrypoints.has(name)) {
								compilation.errors.push(
									new WebpackError(
										"SplitChunksPlugin\n" +
											"Cache group conflicts with entrypoint. " +
											`Both have the same name "${name}".\n` +
											"Found problem while processing " +
											`module ${module.readableIdentifier(
												compilation.requestShortener
											)} ` +
											`for cache group ${cacheGroup.key}\n` +
											"Use a different name for the entrypoint or for the cache group.\n" +
