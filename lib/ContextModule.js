...
class ContextModule extends Module {
	 ...
	 * @returns {TODO} TODO
	 */
	getUserRequestMap(dependencies, moduleGraph) {
	 ...
	 * @returns {TODO} TODO
	 */
	getFakeMap(dependencies, moduleGraph) {
	...
	getSyncSource(dependencies, id, moduleGraph) {
		const map = this.getUserRequestMap(dependencies, moduleGraph);
		const fakeMap = this.getFakeMap(dependencies, moduleGraph);
...
webpackContext.id = ${JSON.stringify(id)};`;
	...
	getWeakSyncSource(dependencies, id, moduleGraph) {
		const map = this.getUserRequestMap(dependencies, moduleGraph);
		const fakeMap = this.getFakeMap(dependencies, moduleGraph);
		const returnModuleObject = this.getReturnModuleObjectSource(fakeMap);
...
module.exports = webpackContext;`;
	...
	getAsyncWeakSource(dependencies, id, moduleGraph) {
		const map = this.getUserRequestMap(dependencies, moduleGraph);
		const fakeMap = this.getFakeMap(dependencies, moduleGraph);
		const returnModuleObject = this.getReturnModuleObjectSource(fakeMap);
...
module.exports = webpackAsyncContext;`;
	...
	getEagerSource(dependencies, id, moduleGraph) {
		const map = this.getUserRequestMap(dependencies, moduleGraph);
		const fakeMap = this.getFakeMap(dependencies, moduleGraph);
		const thenFunction =
			fakeMap !== 9
	...
	getSourceString(asyncMode, { runtimeTemplate, moduleGraph, chunkGraph }) {
		...
		if (asyncMode === "lazy") {
			if (this.blocks && this.blocks.length > 0) {
				return this.getLazySource(this.blocks, id, moduleGraph, chunkGraph);
			}
			return this.getSourceForEmptyAsyncContext(id);
		}
		if (asyncMode === "eager") {
			if (this.dependencies && this.dependencies.length > 0) {
				return this.getEagerSource(this.dependencies, id, moduleGraph);
			}
			return this.getSourceForEmptyAsyncContext(id);
		}
		...
		if (asyncMode === "async-weak") {
			if (this.dependencies && this.dependencies.length > 0) {
				return this.getAsyncWeakSource(this.dependencies, id, moduleGraph);
			}
			return this.getSourceForEmptyAsyncContext(id);
		}
		if (asyncMode === "weak") {
			if (this.dependencies && this.dependencies.length > 0) {
				return this.getWeakSyncSource(this.dependencies, id, moduleGraph);
			}
		}
		if (this.dependencies && this.dependencies.length > 0) {
			return this.getSyncSource(this.dependencies, id, moduleGraph);
		}
