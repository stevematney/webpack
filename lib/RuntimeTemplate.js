...
module.exports = class RuntimeTemplate {
	...
	moduleRaw({ module, chunkGraph, request, weak }) {
		...
		return `__webpack_require__(${this.moduleId({ module, chunkGraph, request, weak })})`;
	}
	 ...
	 * @returns {string} the expression
	 */
	moduleNamespace({ module, chunkGraph, request, strict, weak }) {
		if (!module) {
			return this.missingModule({
				request
			});
	...
	moduleNamespacePromise({
		...
		if (exportsType === "namespace") {
			...
			if (header) {
				getModuleFunction = `function() { ${header}return __webpack_require__.t(${moduleIdExpr}, 3); }`;
			} else {
				getModuleFunction = `__webpack_require__.t.bind(null, ${comment}${idExpr}, 3)`;
			}
		} else if (strict) {
			if (header) {
				getModuleFunction = `function() { ${header}return __webpack_require__.t(${moduleIdExpr}, 1); }`;
			} else {
				getModuleFunction = `__webpack_require__.t.bind(null, ${comment}${idExpr}, 1)`;
			}
		} else {
			if (header) {
				getModuleFunction = `function() { ${header}return __webpack_require__.t(${moduleIdExpr}, 7); }`;
			} else {
	 ...
	 */
	importStatement({ update, module, chunkGraph, request, importVar, originModule, weak }) {
		if (!module) {
